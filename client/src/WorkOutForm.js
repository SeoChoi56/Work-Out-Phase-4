...
function WorkOutForm({ getWorkOuts }) {
  ...
  return (
    <div className="justify-center items-center p-10 flex flex-col max-h-max font-mono bg-[url('/public/image.png')]">
      ...
      <div className="font-bold flex flex-col items-center max-h-max">
        <h4>Search for an Exercise</h4>

        <form id="workouts-search-by-term" onSubmit={handleSearchAPI} className="flex flex-col items-center max-h-max">
          <input type="text" id="workoutSearch" name="name" className="border-2 border-red-500 rounded-md"/>
          <button className="border-red-400 bg-red-400 rounded-md border-4">Search</button>
        </form>
      </div>


      <form id="workouts-by-type-form" className="font-bold flex flex-col items-center max-h-max p-6" onSubmit={callWorkOutApi}>
        <h4>
